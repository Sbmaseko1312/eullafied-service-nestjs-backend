"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.TicketCategoryService = void 0;
const common_1 = require("@nestjs/common");
const typeorm_1 = require("@nestjs/typeorm");
const typeorm_2 = require("typeorm");
const ticket_category_entity_1 = require("./entities/ticket-category.entity");
let TicketCategoryService = class TicketCategoryService {
    ticketCategoryRepository;
    constructor(ticketCategoryRepository) {
        this.ticketCategoryRepository = ticketCategoryRepository;
    }
    async create(createTicketCategoryDto) {
        const category = this.ticketCategoryRepository.create(createTicketCategoryDto);
        return this.ticketCategoryRepository.save(category);
    }
    async findAll() {
        return this.ticketCategoryRepository.find({
            relations: ['tickets'],
        });
    }
    async findOne(id) {
        const category = await this.ticketCategoryRepository.findOne({
            where: { category_id: id },
            relations: ['tickets'],
        });
        if (!category) {
            throw new common_1.NotFoundException(`Ticket Category with ID ${id} not found`);
        }
        return category;
    }
    async update(id, updateTicketCategoryDto) {
        const category = await this.findOne(id);
        Object.assign(category, updateTicketCategoryDto);
        return this.ticketCategoryRepository.save(category);
    }
    async remove(id) {
        const category = await this.findOne(id);
        await this.ticketCategoryRepository.remove(category);
        return category;
    }
};
exports.TicketCategoryService = TicketCategoryService;
exports.TicketCategoryService = TicketCategoryService = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, typeorm_1.InjectRepository)(ticket_category_entity_1.TicketCategory)),
    __metadata("design:paramtypes", [typeorm_2.Repository])
], TicketCategoryService);
//# sourceMappingURL=ticket-category.service.js.map